...
- **A venue error is data, not a process failure.** `run_scraper.py` exits 0 on any *completed* run, even with per-venue errors — the errors live in `scrape_health.json` (status `error`), and the other venues' events are merged and written. Never reintroduce a non-zero exit for venue errors: in Aug 2026 a single MoS 403 exited 1, which blocked `scrape.yml`'s commit step for 4 days — suppressing both the fresh data AND the health report explaining the breakage. CI visibility instead comes from the workflow's post-commit **"Surface venue errors"** step, which reads the *published* health file, writes per-venue details to the Actions run Summary, and exits 1 (red run → GitHub failure email). The commit step runs `if: always()`; all three data outputs are written atomically (`write_json_atomic`) so whatever is on disk is always safe to commit.
- **Staleness is detected by the reader, not the writer** — a dead pipeline can't report itself. `app_health.html` treats `generated_at` older than 26h as the loudest condition on the page (outranking venue errors, whose counts are themselves stale) and links directly to the Action's run history.
